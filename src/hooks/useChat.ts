  ...
  export function useChat(initialSessionId?: string) {
    ...
    const sendMessage = async (query: string, file?: File) => {
      ...
      if (file) {
          ...
          const { response: responseText, image, fileName, session_id } = res;
          ...
          // ✅ Perbarui message terakhir (user) dengan gambar
          setMessages((prev) => {
            const updated = [...prev];
            ...
            return [
              ...
              {
                sender: "Agent",
                text: responseText.replace(/^agent:\s*agent:\s*/i, ""),
                timestamp: new Date(),
              },
            ];
          });
      ...
      async (loadedSessionId: string, loadedMessages: ChatMessage[]) => {
        try {
          ...
          const formattedMessages = loadedMessages.map(msg => ({
            ...
            image: msg.image || undefined,       // ✅ jika disimpan, ini akan muncul
            fileName: msg.fileName || undefined, // ✅ agar preview filename muncul juga
          }));
