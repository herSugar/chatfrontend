...
import { api, getUserBookmarks } from "../services/api";
...
const BookmarkPage: React.FC = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden transition-colors duration-500">
      ...
      <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-500 ease-in-out ${isCollapsed ? 'ml-0' : 'ml-64'} relative`}>
        <div className={`sticky top-0 z-40 backdrop-blur-md ${isDarkMode ? 'bg-black/20 border-white/20' : 'bg-white/20 border-black/20'} border-b`}>
          <div className="flex items-center p-4">
            ...
            <h1 className={`ml-4 text-2xl font-bold ${themeStyles.heading}`}>My Bookmark Sessions</h1>
          </div>
        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full">
                    {bookmarks.slice(0, visibleCount).map((session, index) => (
                      ...
                      >
                        ...
                        <div className={`absolute bottom-3 right-3 ${themeStyles.subheading} text-lg font-bold opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0`}>
                          →
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="flex justify-center mt-6">
                      <button
                        ...
                        className={`px-4 py-2 text-sm rounded-lg transition-all duration-300 
                          ${isDarkMode ? 'bg-orange-500/10 hover:bg-orange-500/20' : 'bg-orange-100 hover:bg-orange-200'} 
                          ${themeStyles.text} border ${isDarkMode ? 'border-orange-400/40' : 'border-orange-500/40'}
                        `}
                      >
