...
export default function ChatContainer({ sessionId }: Props) {
  ...
  useEffect(() => {
    ...
    if (sessionId !== prevSessionId && prevSessionId !== undefined) {
      console.log("Session changed, resetting...", {
        prevSessionId,
        sessionId,
      });
  ...
  const handleSaveBookmark = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/history/save/${firebaseUid}/${currentSessionId}`,
        {
          method: "POST",
          headers: {
  ...
  console.log(
    "ChatContainer render - sessionId:",
    sessionId,
    "messages count:",
    messages.length,
    "sessionLoading:",
              ...
              {effectiveSessionId && (
                <button
                  ...
                  className={`ml-4 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${
                    isDarkMode
                      ? "bg-orange-500/20 hover:bg-orange-600/30 text-orange-200"
          ...
          <div className="flex flex-col h-full min-h-0 container mx-auto pr-2">
            ...
            >
              <div className="flex flex-col">
                {sessionLoading && (
                  <div
                    className={`${themeStyles.text} text-center italic mt-4`}
                  >
                    <p>Loading session...</p>
                    <div className="mt-2">
                      <div
                ...
                {!sessionId &&
                  ...
                  !sessionLoading && (
                    <div className={`${themeStyles.text} text-center mt-20`}>
                      <h2
                        className={`text-4xl font-bold mb-4 ${themeStyles.heading}`}
                      >
                        Hi, User!
                      </h2>
                      <p className={`mb-2 ${themeStyles.text}`}>
                        Start a conversation by typing your message below.
                      </p>
                      <p className={`text-sm ${themeStyles.mutedText}`}>
                        You can also upload images to get travel information
                        about Bali destinations!
                      </p>
                    </div>
                  )}
                  ...
                  <div
                    className={`backdrop-blur-sm ${
                      isDarkMode
                        ...
                        : "bg-red-100/60 border-red-500/50"
            ...
            <div className="sticky bottom-0 z-30 p-4 backdrop-blur-md bg-transparent">
              ...
              {selectedFile && (
                <div className="max-w-4xl mx-auto w-full mb-2">
                  <div
                    className={`backdrop-blur-lg rounded-3xl p-3 border ${
                      isDarkMode
                        ? "bg-black/20 border-white/20"
                        : "bg-white/20 border-black/20"
                    }`}
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`${themeStyles.text} text-sm truncate`}>
                          {selectedFile.name}
                        </p>
                        <p className={`${themeStyles.mutedText} text-xs`}>
              ...
              {/* Input Area */}
              <div
                className={`max-w-4xl mx-auto w-full flex gap-2 p-2 border ${
                  isDarkMode
                    ? "bg-black/20 border-white/20"
                    : "bg-white/20 border-black/20"
                } backdrop-blur-lg rounded-3xl shadow-lg items-center`}
              >
                {/* Hidden file input */}
                <input
                  ref={fileInputRef}
                ...
                <button
                  ...
                  className={`p-3 bg-transparent rounded-full hover:${
                    themeStyles.buttonSecondary
                      .replace("bg-", "hover:bg-")
                      .split(" ")[0]
                  } transition-colors flex items-center justify-center group relative`}
                  disabled={loading || sessionLoading}
                  title="Upload gambar
Tipe gambar: JPEG, JPG, PNG, GIF, WEBP (maks 10MB)"
                ...
                <button
                  ...
                  } rounded-full p-3 flex items-center justify-center transition-colors group relative`}
                  onClick={onSend}
                  disabled={
                    loading ||
                    sessionLoading ||
