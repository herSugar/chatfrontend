    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className={`absolute w-1.5 h-1.5 ${isDarkMode ? 'bg-orange-400/20' : 'bg-orange-600/20'} rounded-full animate-pulse`}
            style={{
      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        <div className={`flex items-center justify-center py-6 h-40 border-b ${isDarkMode ? 'border-white/20' : 'border-black/20'}`}>
          ...
          <nav>
            <ul className="space-y-2">
              {menuItems.map((item, index) => (
                <li key={item.id} className="animate-slide-in" style={{ animationDelay: `${index * 0.05}s` }}>
                  <button
                    ...
                    className={`
                      ...
                      ${
                        activeItem === item.id
                          ? `${isDarkMode ? 'bg-orange-500/20 border-orange-400/40' : 'bg-orange-100/60 border-orange-500/40'} border shadow-lg`
                          : `${isDarkMode ? 'hover:bg-white/10 border-white/10' : 'hover:bg-black/10 border-black/10'} border hover:shadow-md`
                      }
                    `}
                  >
                    {/* Icon */}
                    <span className="text-lg flex-shrink-0 transition-transform duration-300 group-hover:scale-110">
                      {item.icon}
                    </span>
                    ...
                    {/* Active indicator */}
                    {activeItem === item.id && (
                      <div className={`absolute right-2 w-2 h-2 rounded-full ${isDarkMode ? 'bg-orange-400' : 'bg-orange-600'} animate-pulse`} />
                    )}
                  </button>

                  {/* Logout Button - appears after Profile */}
                  {item.id === 'profile' && (
                    <button
                      onClick={() => {
                        localStorage.clear();
                      ...
                      className={`
                        relative group mt-2 w-full text-left p-3 rounded-xl cursor-pointer 
                        transition-all duration-300 transform hover:scale-[1.02]
                        flex items-center gap-3 border border-red-400/40
                        backdrop-blur-sm bg-red-500/20 hover:bg-red-500/30 text-red-900 hover:shadow-md
                      `}
                    >
                      <span className="text-lg flex-shrink-0 transition-transform duration-300 group-hover:scale-110">
                        🚪
        ...
        {/* Footer decoration */}
        <div className={`p-3 border-t ${isDarkMode ? 'border-white/20' : 'border-black/20'}`}>
          <div className={`w-full h-1 ${isDarkMode ? 'bg-orange-400/30' : 'bg-orange-600/30'} rounded-full`} />
        </div>
      </div>
      ...
      <style>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-track {
          background: ${isDarkMode ? 'rgba(31, 41, 55, 0.2)' : 'rgba(243, 244, 246, 0.2)'};
          border-radius: 3px;
        }
